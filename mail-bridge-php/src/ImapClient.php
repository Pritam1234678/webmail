...
final class ImapClient
{
    ...
    public function setFlag(int $uid, string $flag, bool $enable): void
    {
        $action = $enable ? '+FLAGS' : '-FLAGS';
        $this->executeSimple("UID STORE {$uid} {$action} ({$flag})");
    }

    public function moveMessage(int $uid, string $targetFolder): void
    {
        $safeFolder = $this->quote($targetFolder);
        // Try UID MOVE (IMAP extension) or fallback to COPY + STORE \Deleted
        try {
            $this->executeSimple("UID MOVE {$uid} {$safeFolder}");
        } catch (Throwable $e) {
            $this->executeSimple("UID COPY {$uid} {$safeFolder}");
            $this->setFlag($uid, '\\Deleted', true);
            $this->executeSimple("EXPUNGE");
        }
    }
    ...
    public function appendMessage(string $folderName, string $message): void
    {
        ...
        while (($line = $this->readLine()) !== null) {
            if (strpos($line, $tag . ' ') === 0) {
                if (stripos($line, 'OK') === false) throw new RuntimeException('IMAP append failed');
                return;
            }
        }
    }
    ...
    public function logout(): void
    {
        if (is_resource($this->stream)) {
            try { $this->executeSimple('LOGOUT'); } catch (Throwable $e) {}
            fclose($this->stream);
        }
    }
    ...
    {
        $response = $this->execute($command);
        if (!$response['ok']) throw new RuntimeException('IMAP command failed: ' . $command);
    }
    ...
    private function parseFetchBlocks(array $blocks): array
    {
        ...
        foreach ($blocks as $block) {
            $uid = null; $size = null; $flags = []; $rawHeader = ''; $rawBody = '';
            if (preg_match('/UID (\d+)/i', $block, $matches)) $uid = (int) $matches[1];
            if (preg_match('/RFC822\.SIZE (\d+)/i', $block, $matches)) $size = (int) $matches[1];
            if (preg_match('/FLAGS \((.*?)\)/i', $block, $matches)) $flags = array_values(array_filter(explode(' ', trim($matches[1]))));
            if (preg_match_all('/BODY\[(.*?)\] \{(\d+)\}\r\n/i', $block, $matches, PREG_OFFSET_CAPTURE)) {
                foreach ($matches[0] as $index => $match) {
                    ...
                    if (str_contains($partName, 'HEADER')) $rawHeader = $content;
                    elseif (str_contains($partName, 'TEXT') || $partName === '') $rawBody = $content;
                }
            }
            if (!$rawHeader && preg_match('/\r\n(From: .*?)\)\r\n$/si', $block, $matches)) $rawHeader = $matches[1];
            $headers = $this->parseHeaders($rawHeader);
            $messages[] = [
                'uid' => $uid, 'size' => $size, 'flags' => $flags,
    ...
    private function parseHeaders(string $raw): array
    {
        ...
        foreach (preg_split("/\r\n/", $raw) as $line) {
            ...
            if (preg_match('/^\s+/', $line) && $current) {
                $headers[$current] .= ' ' . trim($line); continue;
            }
    ...
    private function extractMessageBody(string $rawHeader, string $rawBody): ?string
    {
        ...
        if ($parsed['html'] !== '') return trim($parsed['html']);
        if ($parsed['text'] !== '') return nl2br(htmlspecialchars(trim($parsed['text'])));
    ...
    private function parseMimePart(string $contentType, string $encoding, string $body): array
    {
        ...
        if (stripos($contentType, 'multipart/') === false) {
            ...
            if (stripos($contentType, 'text/html') !== false) $html = $decoded;
            else $text = $decoded;
            return ['html' => $html, 'text' => $text];
        }
        if (preg_match('/boundary="?([^";\s]+)"?/i', $contentType, $matches)) {
            $boundary = $matches[1]; $parts = explode('--' . $boundary, $body);
    ...
    {
        if (strpos($encoding, 'quoted-printable') !== false) return quoted_printable_decode($data);
        if (strpos($encoding, 'base64') !== false) return base64_decode(trim($data));
        return $data;
    }
    ...
    private function readLine(): ?string { $line = fgets($this->stream); return $line === false ? null : $line; }
