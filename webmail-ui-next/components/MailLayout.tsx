    ...
    <aside style={{
      ...
      <div style={{ padding: '20px 20px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${T.outline}` }}>
        ...
        <div style={{
          ...
          background: T.surfaceHigh, border: `1px solid ${T.outline}`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontFamily: 'Hanken Grotesk', fontSize: 11, fontWeight: 600,
          color: T.onSurface, letterSpacing: '0.05em',
        }}>{initials}</div>
      </div>
      ...
      <div style={{ padding: '16px 16px 8px' }}>
        <button
          ...
          style={{
            ...
            padding: '12px 16px', background: T.surfaceHigh,
            border: `1px solid ${T.outline}`, borderRadius: 2,
            color: T.onSurface, fontFamily: 'Hanken Grotesk',
          ...
          return (
            <button
              ...
              style={{
                ...
                transition: 'all 0.2s cubic-bezier(0.2,0,0,1)', marginBottom: 2,
              }}
              onMouseEnter={e => { if (!isActive) (e.currentTarget as HTMLElement).style.color = T.onSurface }}
              onMouseLeave={e => { if (!isActive) (e.currentTarget as HTMLElement).style.color = T.onSurfaceVar }}
            >
...
function MessageList() {
  const { messages, selectedMessage, setSelectedMessage, loading, selectedFolder, setSearchQuery, searchQuery, refreshMessages } = useMail()
            ...
            {(['focused', 'other'] as const).map(t => (
              <button
                ...
                style={{
                  ...
                  fontFamily: 'Hanken Grotesk', fontSize: 14, fontWeight: 500,
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            style={{
              ...
              background: T.surfaceMid, border: `1px solid ${T.outline}`,
              borderRadius: 2, color: T.onSurface,
              fontFamily: 'Hanken Grotesk', fontSize: 14, outline: 'none',
              transition: 'border-color 0.2s',
            }}
            onFocus={e => (e.target as HTMLElement).style.borderColor = 'rgba(233,195,73,0.4)'}
            onBlur={e => (e.target as HTMLElement).style.borderColor = T.outline}
          />
        </div>
      </div>
          ...
          messages.map((msg, i) => {
            ...
            return (
              <motion.button
                ...
                transition={{ delay: i * 0.03, duration: 0.3, ease: [0.2, 0, 0, 1] }}
                onClick={() => setSelectedMessage(msg)}
                style={{
                  width: '100%', textAlign: 'left', padding: '20px 24px',
                  background: isActive ? T.surfaceMid : 'transparent',
                  border: 'none', borderBottom: `1px solid ${T.outline}`,
                  borderLeft: isActive ? `3px solid ${T.gold}` : '3px solid transparent',
                  cursor: 'pointer', display: 'flex', gap: 14, fontFamily: 'inherit',
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontFamily: 'Hanken Grotesk', fontSize: 14, fontWeight: msg.unread ? 500 : 400, color: T.onSurface, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginBottom: 4 }}>
  ...
  if (!selectedMessage) {
    return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: T.bg, color: T.onSurfaceVar }}>
        <span className="material-symbols-outlined" style={{ fontSize: 48, marginBottom: 16, opacity: 0.2, fontVariationSettings: "'FILL' 0, 'wght' 100" }}>mail</span>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '48px 64px' }}>
            <div style={{ marginBottom: 32 }}>
              <h1 style={{
                ...
                fontSize: 40, fontWeight: 600, lineHeight: 1.25,
                letterSpacing: '-0.01em', color: T.onSurface,
                maxWidth: 680,
              }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16, marginBottom: 40, paddingBottom: 40, borderBottom: `1px solid ${T.outline}` }}>
              <div style={{
                ...
                background: T.surfaceHigh, border: `1px solid ${T.outline}`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontFamily: 'Hanken Grotesk', fontSize: 13, fontWeight: 600, color: T.gold, letterSpacing: '0.05em',
              }}>
            ...
            <div style={{ fontFamily: 'Hanken Grotesk', fontSize: 16, lineHeight: 1.85, color: '#c0bdb8', maxWidth: 680, letterSpacing: '0.01em' }}>
              {isHtml ? (
                ...
                <pre style={{ whiteSpace: 'pre-wrap', fontFamily: 'inherit', fontSize: 'inherit', lineHeight: 'inherit', color: 'inherit' }}>
            ...
            <div style={{ maxWidth: 680, marginTop: 48 }}>
              {!showReply ? (
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 20px', background: T.surfaceMid, border: `1px solid ${T.outline}`, borderRadius: 2, color: T.onSurfaceVar, fontFamily: 'Hanken Grotesk', fontSize: 13, cursor: 'pointer', transition: 'all 0.2s' }}
                  onMouseEnter={e => (e.currentTarget as HTMLElement).style.borderColor = 'rgba(233,195,73,0.3)'}
                  onMouseLeave={e => (e.currentTarget as HTMLElement).style.borderColor = T.outline}
                >
                ...
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} style={{ border: `1px solid rgba(233,195,73,0.25)`, borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{ padding: '10px 16px', background: T.surfaceMid, borderBottom: `1px solid ${T.outline}`, fontFamily: 'Hanken Grotesk', fontSize: 12, color: T.onSurfaceVar }}>
                  ...
                  <textarea
                    ...
                    autoFocus
                    style={{ width: '100%', padding: '20px', background: '#0a0a0b', color: T.onSurface, fontFamily: 'Hanken Grotesk', fontSize: 15, lineHeight: 1.7, outline: 'none', resize: 'none', border: 'none', minHeight: 140 }}
                  />
                  <div style={{ padding: '12px 16px', background: T.surfaceMid, borderTop: `1px solid ${T.outline}`, display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                    <button onClick={() => setShowReply(false)} style={{ padding: '8px 16px', background: 'none', border: `1px solid ${T.outline}`, borderRadius: 2, color: T.onSurfaceVar, fontFamily: 'Hanken Grotesk', fontSize: 12, cursor: 'pointer' }}>Discard</button>
                    <button disabled={!replyText.trim()} style={{ padding: '8px 18px', background: T.gold, border: 'none', borderRadius: 2, color: '#1c1b1b', fontFamily: 'Hanken Grotesk', fontSize: 12, fontWeight: 600, cursor: replyText.trim() ? 'pointer' : 'not-allowed', display: 'flex', gap: 8, alignItems: 'center', opacity: replyText.trim() ? 1 : 0.5 }}>
                      Send <span className="material-symbols-outlined" style={{ fontSize: 14 }}>send</span>
                    </button>
                  </div>
                </motion.div>
              )}
...
const toolbarBtnStyle: React.CSSProperties = {
  ...
  borderRadius: 2, color: '#c4c7c7', display: 'flex',
